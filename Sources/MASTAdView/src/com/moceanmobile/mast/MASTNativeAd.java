...
public final class MASTNativeAd implements AdRequest.Handler {
    ...
    public interface LogListener {
         ...
         * @return
         */
        public boolean onLogEvent(MASTNativeAd nativeAd, String event, LogLevel eventLevel);
    }
    ...
    private void internalUpdate(boolean defaulted) {
        ...
        try {
            ...
            mAdRequest = AdRequest.create(Defaults.NETWORK_TIMEOUT_SECONDS, mNetworkUrl, mUserAgent, args,
                    mRequestedNativeAssets, this, true);
    ...
    public List<AssetResponse> getNativeAssets() {
        if (mNativeAdDescriptor != null && mNativeAdDescriptor.getNativeAssetList() != null
            ...
            switch (trackerType) {
                case IMPRESSION_TRACKER:
                    AdTracking.invokeTrackingUrl(Defaults.NETWORK_TIMEOUT_SECONDS,
                            mNativeAdDescriptor.getNativeAdImpressionTrackers(), mUserAgent);
                    break;
                case CLICK_TRACKER:
                    AdTracking.invokeTrackingUrl(Defaults.NETWORK_TIMEOUT_SECONDS,
                            mNativeAdDescriptor.getNativeAdClickTrackers(), mUserAgent);
                    break;
            }
        }
    }
                ...
                if (mBrowserDialog == null) {
                    mBrowserDialog = new BrowserDialog(mContext, url, new BrowserDialog.Handler() {
                        ...
                        public void browserDialogOpenUrl(BrowserDialog browserDialog, String url, boolean dismiss) {
                            /*
                             * Since internal browser is unable to handle the
                             * url, open the url in native browser.
                             */
            ...
            if (mNativeAdDescriptor.isTypeMediation()) {
                try {
                    if (!mOverrideAdapter) {
						...
						if (TextUtils.equals(MEDIATION_FAN_NAME,
								mNativeAdDescriptor.getMediation())) {
							className = MEDIATION_FAN_ADAPTER_CLASS;
						} else if (TextUtils.equals(MEDIATION_MOPUB_NAME,
								mNativeAdDescriptor.getMediation())) {
							className = MEDIATION_MOPUB_ADAPTER_CLASS;
						} else {
							className = mNativeAdDescriptor.getMediation();
						}

						// Launch the adapter with the class name received.
						mBaseAdapter = MASTBaseAdapter.getAdapterForClass(
								className, mContext);

						if (mBaseAdapter != null) {
							mBaseAdapter.init(mContext, mNativeAdDescriptor,
									adapterListener,mRequestedNativeAssets);
							...
							// Make new request notifying that third party
							// partner defaulted
							thirdpartyPartnerDefaulted();
							logEvent(
									"Adapter not found for class " + className,
									LogLevel.Error);
						}
						// @formatter:on
                    } else {
    ...
    private void fireCallback(CallbackType callbackType, Exception ex, ThirdPartyDescriptor thirdPartyDescriptor) {
            ...
            switch (callbackType) {
                ...
                case ThirdPartyReceived:
                    if (thirdPartyDescriptor != null) {
                        mListener.onReceivedThirdPartyRequest(this, thirdPartyDescriptor.getProperties(),
                                thirdPartyDescriptor.getParams());
                    } else {
                        fireCallback(CallbackType.NativeFailed, new Exception("Third party response is invalid"),
